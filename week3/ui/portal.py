...
def callLLM(message, model="gpt-oss:20b"):
    """调用 LLM 接口"""
    response = requests.post(OLLAMA_API_URL + "api/generate", json={
        "model": model,
        "prompt": message,
        "stream": False
    })
...
class SearchUI:
    ...
    def setup_ui(self):
        ...
        with gr.Blocks(title="倒排索引检索系统") as self.interface:
            ...
            **教学演示系统：**
            ...
            # 基于 tf-idf 的 RAG 功能
            with gr.Accordion("📖 基于 TF-IDF 的 RAG 功能", open=False):
                rag_input = gr.Textbox(
                    label="请输入您的问题",
                    placeholder="请输入RAG查询内容...",
                    lines=1
                )
                rag_btn = gr.Button("📚 执行 RAG 查询", variant="primary")
                rag_output = gr.HTML(
                    label="RAG 结果",
                    value="<p>请输入RAG查询内容查看结果...</p>"
                )
            # RAG 查询事件
            rag_btn.click(
                fn=self.rag_query,
        ...
        try:
            ...
            if sort_mode == "tfidf":
                ...
                if any(x[2] is not None for x in results):
                    results = sorted(results, key=lambda x: (
                        x[2] if x[2] is not None else 0), reverse=True)
                else:
                    results = sorted(results, key=lambda x: x[1], reverse=True)
            ...
            for position, result in enumerate(results):
                ...
                plain_summary = summary.replace(
                    '<span style="background-color: yellow; font-weight: bold;">', '').replace('</span>', '')
    ...
    def get_history_html(self):
        ...
        for record in records:
            label = "<span style='color: #fff; background: #28a745; padding:2px 8px; border-radius:4px;'>点击</span>" if record[
                'clicked'] else "<span style='color: #fff; background: #6c757d; padding:2px 8px; border-radius:4px;'>未点击</span>"
            html_content += f"""
    ...
    def rag_query(self, question: str):
        ...
        if not question.strip():
            return "<p>请输入RAG查询内容...</p>"

        try:
            # 先retrieve召回文档ID
            doc_ids = self.search_engine.retrieve(question.strip(), top_k=5)
            # 再rank精排
            results = self.search_engine.rank(question.strip(), doc_ids, top_k=10)
            # 按TF-IDF分数排序
            results = sorted(results, key=lambda x: x[1], reverse=True)
            first_doc_id, first_tfidf_score, first_ctr_score, first_summary = results[0]
            first_content = self.search_engine.get_document(first_doc_id)
            ...
            answer = llm_response.get('response', '未能生成回答，请重试。')

            return answer

        except Exception as e:
            return f"<p>RAG查询失败: {e}</p>"
